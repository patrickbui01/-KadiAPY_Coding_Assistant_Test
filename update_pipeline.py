...
class UpdatePipeline:
    def __init__(self, gitlab_config_path, huggingface_config_path, dataset_kadi_apy_doc_params_path, dataset_kadi_apy_library_params_path, update_history_path):
        self.gitlab_config_path = gitlab_config_path
        self.huggingface_config_path = huggingface_config_path
        self.dataset_kadi_apy_doc_params_path = dataset_kadi_apy_doc_params_path
        self.dataset_kadi_apy_library_params_path = dataset_kadi_apy_library_params_path
        self.update_history_path = update_history_path
        
        # Preloading configurations for efficiency
        self.gitlab_config = ConfigLoader(self.gitlab_config_path).load()
        self.huggingface_config = ConfigLoader(self.huggingface_config_path).load()
        self.kadi_doc_params = ConfigLoader(self.dataset_kadi_apy_doc_params_path).load()
        self.kadi_library_params = ConfigLoader(self.dataset_kadi_apy_library_params_path).load()
    ...
    def start_update_pipeline(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.upload_folder_to_hf(temp_dir)
            time.sleep(2)  # Allow time for resource release
    ...
    def update_vectorstore_history(self, version_number):
        ...
        new_entry = {
            ...
            "deployed_to_hf_repo": self.huggingface_config["hf_repo_id"]
        }
    ...
    def delete_vectorstore_folder_from_huggingface(self):
        hf_repo_id = self.huggingface_config["hf_repo_id"]
        hf_repo_type = self.huggingface_config["hf_repo_type"]
        hf_vectorstore_path = self.huggingface_config["hf_vectorstore_path"]
    ...
    def chunk_kadiAPY_doc_dataset(self, doc_files_content, doc_files_path):
        chunk_size = self.kadi_doc_params["chunking"]["chunking_size"]
        chunk_overlap = self.kadi_doc_params["chunking"]["chunking_overlap"]
        dataset_name = self.kadi_doc_params["dataset"]
    ...
    def chunk_kadiAPY_library_files_dataset(self, kadiAPY_library_files_content, kadiAPY_library_files_content_path):
        dataset_name = self.kadi_library_params["dataset"]
    ...
    def get_kadiAPY_doc_dataset(self, repo_zip_filepath):
        directories_of_kadi_apy_docs = self.kadi_doc_params.get("folder", [])
    ...
    def get_kadiAPY_library_dataset(self, repo_zip_filepath):
        directory_of_kadi_apy_library_source_code = self.kadi_library_params.get("folder", [])
    ...
    def get_deployed_vectorstore_version_tag(self):
        ...
        first_entry = data["update_history"][0]  # Extract the first entry
        return first_entry["project_release_version"]  # Correctly return the value

    def get_kadiAPY_latest_release_version_tag(self):
        gitlab_api_url = self.gitlab_config["api_url"]
        gitlab_project_id = self.gitlab_config["project id"]
